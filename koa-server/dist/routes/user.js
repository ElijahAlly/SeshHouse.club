...
router.post('/api/user', async (ctx) => {
    const { first_name, last_name, username, email, phone_number, password } = ctx.request.body;
 ...
 *         description: Internal Server Error
 */
router.put('/api/user', isAuthenticated_1.default, async (ctx) => {
    ...
    try {
        ...
        const updatedUser = await db_1.default.query('SELECT * FROM users where id = ' + id);
        ctx.status = 200;
        ctx.body = {
            code: 200,
            status: 'success',
 ...
 *         description: Internal Server Error
 */
router.delete('/api/user/:id', isAuthenticated_1.default, async (ctx) => {
    const queryParams = ctx.query;
...
router.post('/api/login', async (ctx) => {
    ...
    try {
        let result = await db_1.default.query('SELECT * FROM users WHERE username = $1', [username_or_email]);
        let user = result.rows[0];
        if (!user) {
            result = await db_1.default.query('SELECT * FROM users WHERE email = $1', [username_or_email]);
            user = result.rows[0];
            if (!user) {
                ctx.status = 404;
                ctx.body = {
        ...
        // Generate JWT token
        const token = jsonwebtoken_1.default.sign({ id: user.id }, config_1.default.JWT_SECRET, { expiresIn: '7d' });
        ctx.status = 200;
        ctx.body = {
            code: 200,
            status: 'success',
 ...
 *         description: Internal Server Error
 */
router.post('/api/logout', isAuthenticated_1.default, async (ctx) => {
    ctx.session = null;
    ctx.status = 200;
    ctx.body = {
        code: 200,
